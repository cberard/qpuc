...
async def read_user_questions_to_answer(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    db_questions = crud_questions.get_user_questions_to_answer(db=db, user_id=user_id, skip=skip, limit=limit)   
...
async def read_user_question_to_answer_today(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    db_user = crud_users.get_user(db=db, user_id=user_id)
    if db_user is None:
       raise HTTPException(status_code=404, detail="User not found")

    db_question = crud_questions.get_user_questions_to_answer_today(db=db, user_id=user_id)   
